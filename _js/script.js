...
var TEMPLATES = {
    ...
    SAMPLES: {
        filename: 'templates/samples.html',
        target: '#samples-items'
    },
    NUMBERS: {
        filename: 'templates/numbers.html',
        target: ''
    },
    TEXT: {
        filename: 'templates/text.html',
        target: ''
    },
    DATE: {
        filename: 'templates/date.html',
        target: ''
    },
    NAV_ITEMS: {
        filename: 'templates/nav_items.html',
        target: '#affix-nav'
    },
    NAV_ITEM: {
        filename: 'templates/nav_item.html',
        target: '#navigation'
    },
    QUESTIONS: {
        filename: 'templates/questions.html',
        target: ''
    },
    SUGGESTIONS: {
        filename: 'templates/suggestions.html',
        target: ''
    },
    INTRO: {
...
function loadSamples() {
    $.getJSON("config/samples.json", function(data) {
        //        console.log(data);
        TEXT_FILES = data;
        renderTemplate(TEMPLATES.SAMPLES, {
            files: TEXT_FILES
        });
    });
...
function showCards(data) {
    ...
    console.log('est. object count =' + data.columnCount + ' *' + data.columnLength + '=' + data.columnCount * data.columnLength)
    if (imageMode)
...
function loadSuggestions(d) {
    ...
    renderTemplate(TEMPLATES.SUGGESTIONS, {
        suggestions: d.suggestions
    }, +'#' + d.id + '-suggestions', true, false);
}

function loadQuestions(d) {
    renderTemplate(TEMPLATES.QUESTIONS, {
        data: d
    }, '#' + d.id + '-questions', true, false);
...
function refreshNavigation() {
    ...
    var cards = $('.mrc-card');
    $('#navigation').remove();
    renderTemplate(TEMPLATES.NAV_ITEMS, {
        cards: cards
    }, TEMPLATES.NAV_ITEMS.target, false, false)
